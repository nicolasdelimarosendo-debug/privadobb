...
const ADMIN_KEY = "c99d9f8c6f690afcb4431a50d658e4ac1d2bbb561874f3390f587863ae4cc2fb"
...
export function KeyLogin({ onLogin }: KeyLoginProps) {
  ...
  const handleKeySubmit = async () => {
    ...
    const foundKey = storedKeys.find((k: any) => k.key === key.trim() && !k.used)

    if (foundKey) {
      // Marca como usada
      const updatedKeys = storedKeys.map((k: any) =>
        k.key === key.trim() ? { ...k, used: true } : k
      )
      localStorage.setItem("generatedKeys", JSON.stringify(updatedKeys))
      onLogin(key.trim(), false)
    } else {
      setError("Key inválida ou já utilizada")
    }
  }

  const handleAdminSubmit = () => {
    if (adminKey === ADMIN_KEY) {
      onLogin(ADMIN_KEY, true)
    } else {
      setError("Key de admin inválida")
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-3">
              <Button
                ...
                className="flex-1 h-12 text-lg font-bold bg-[#22c55e] hover:bg-[#16a34a] text-white shadow-lg shadow-[#22c55e]/30 transition-all hover:shadow-xl hover:scale-105"
              >
                Confirmar Key
              </Button>
              <Button
                ...
                variant="outline"
                className="h-12 px-6 text-lg font-bold border-2 border-[#22c55e] text-[#22c55e] hover:bg-[#22c55e]/10"
              >
                Admin
              </Button>
            </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <Input
                ...
                placeholder="Digite a key de admin..."
                className="h-12 text-lg border-2 focus:border-[#22c55e] bg-muted/50"
            ...
            <div className="flex gap-3">
              <Button
                ...
                className="flex-1 h-12 text-lg font-bold bg-[#22c55e] hover:bg-[#16a34a] text-white shadow-lg shadow-[#22c55e]/30"
              >
                Entrar como Admin
              </Button>
              <Button
                ...
                variant="outline"
                className="h-12 px-6 text-lg font-bold border-2"
              >
                Voltar
              </Button>
            </div>
